  ...
  return (
    <div className="relative ">
      ...
      <div className="absolute right-2 top-0 bottom-0 flex justify-center items-center gap-x-3">
        <RxCross2 className={`cursor-pointer duration-700 ${text ? "" : "w-0"}`} onClick={()=> {
          setText("")
          setSingleScanCode("")
        }} size={25}></RxCross2>
        <CiSearch className="hover cursor-pointer hover:text-red-500" onClick={() => setSingleScanCode(text)} size={40}></CiSearch>
      </div>
    </div>
  );
};
