...
class ProductReportController extends Controller
{
    ...
    public function __invoke($time_range = null, $start_date = null, $end_date = null)
    {
        ...
        $query = DB::table('products')
            ...
            AVG(sale_items.tax) as average_vat, SUM(sale_items.quantity * sale_items.average_rate)  as total_sold_price_without_vat, MAX(sales.created_at) as last_sale_date')
            ->groupBy('product_name');
        // Apply date filters based on the time range
        if ($timeRange == 1) {
            $query->whereDate('sales.created_at', '=', now()->toDateString());
        } elseif ($timeRange == 7) {
            $query->whereBetween('sales.created_at', [now()->subDays(7), now()]);
        } elseif ($timeRange == 31) {
